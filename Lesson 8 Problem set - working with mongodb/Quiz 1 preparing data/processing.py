        ...
        for line in reader:
            ...
            for field in process_fields:
                if fields[field] == 'label':
                    entry[fields[field]] = line[field].split(' ')[0]
                
                elif fields[field] == 'name':
                    if line[field] == 'NULL':
                        entry[fields[field]] = line['rdf-schema#label'].split(' ')[0]
                    elif line[field].isalnum():
                        entry[fields[field]] = check_null_value(line[field])
                    else: 
                        entry[fields[field]] = line['rdf-schema#label'].split(' ')[0]
                    
                elif fields[field] in ['uri', 'description']:
                    entry[fields[field]] = check_null_value(line[field])
                    
                elif fields[field] == 'synonym':
                    entry[fields[field]] = check_null_value(parse_array(line[field]))
                    
                else:
                    entry['classification'][fields[field]] = check_null_value(line[field])
